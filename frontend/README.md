...
`FRONTEND_GATEWAY_PUBLIC_URL`. It can select any currently registered gateway
agent, including `Release Scout`. In local Docker Compose runs, Release Scout
requires the optional `local-llm` profile because it depends on Ollama and the
upcoming releases MCP server. `CHATKIT_API_PATH` is still written to `config.js`
for the gateway's ChatKit-oriented adapter.
